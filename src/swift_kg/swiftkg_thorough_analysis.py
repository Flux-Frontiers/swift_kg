#!/usr/bin/env python3
"""
SwiftKG Thorough Repository Analysis Tool

Performs comprehensive architectural analysis of Swift repositories
using SwiftKG's graph traversal capabilities. Analyzes:
...
- Entry point defaults: resolves ``repo_root`` and defaults ``db_path``/``vectors_path``
  to ``.swiftkg/graph.sqlite`` and ``.swiftkg/vectors.sqlite``.
...
#: Members the runtime, standard library or a UI framework invokes, rather
#: than repository code. Swift reaches a large share of its surface through
#: protocol witnesses and framework dispatch, so a zero-caller count says
#: nothing about these — counting them as orphans buries the real dead code.
...
#: Order in which node kinds appear in the doc-coverage table.
_COVERAGE_KIND_ORDER: tuple[str, ...] = (
    "protocol",
    "class",
    "struct",
    "actor",
...
class FunctionMetrics:
    ...
    :param kind: Node kind (function, method, class, struct, protocol, …)
    ...
    :param docstring: Doc-comment text if available
    """
...
@dataclass
class ModuleMetrics:
    """Metrics for a module (single Swift file).
...
class SwiftKGAnalyzer:
    """Thorough Swift repository analyzer using SwiftKG.

    :param kg: SwiftKG instance (built KG required for useful results)
    ...
    def __init__(
        ...
        self.doc_comment_coverage: dict = {}
    ...
    def _run_phase(self, num: int, name: str, fn: Callable[[], None]) -> None:
        ...
        ``swiftkg build-sqlite``, a supported command that deliberately builds
    ...
    def run_analysis(
        ...
        *,
        ...
        try:
            ...
            self._run_phase(10, "doc-comment coverage", self._analyze_doc_comment_coverage)
            self._run_phase(11, "Type hierarchy & conformance", self._analyze_inheritance)
    ...
    def _compute_coderank(self) -> None:
        ...
        try:
            from swift_kg.coderank import (  # noqa: PLC0415
                build_code_graph,
                compute_coderank,
            )
    ...
    def _is_swift_entry_point(self, node: dict) -> bool:
        """Return True if this node is a framework-driven entry point.

        Swift has an unusually large population of declarations with zero
        internal callers *by design*, because so much of the language's
        surface is reached through protocol witnesses and framework
        dispatch rather than through direct calls:

        - SwiftUI's ``body``, and anything under a ``Views/`` tree
        - ``@main`` / ``App`` / ``AppDelegate`` / ``SceneDelegate`` entry points
        - UIKit and AppKit lifecycle methods (``viewDidLoad``, …)
        - XCTest and swift-testing methods (``test*``, ``setUp``, ``tearDown``)
        - ``@objc`` selector targets and IBAction handlers
        - protocol witnesses (``description``, ``encode(to:)``, ``hash(into:)``)
        - a ``public`` or ``open`` declaration, whose callers are by
          definition outside the module being indexed

        Counting these as orphans would bury the genuinely dead code that the
        orphan phase exists to find.
        """
        name = node.get("name", "")
        ...
        # A public API's callers live outside this repository by design.
        if visibility in ("public", "open"):
            return True
        if kind in ("class", "struct", "enum", "protocol", "actor", "extension"):
            return True
        if name in _RUNTIME_MEMBER_NAMES:
            return True
        if name.startswith(("test", "on", "handle", "did", "will")):
            return True
        if "/tests/" in lowered or lowered.endswith(("tests.swift", "test.swift", "spec.swift")):
            return True
        if any(part in lowered for part in ("/views/", "/scenes/", "/screens/")):
            return True
        if any(
            module.endswith(suffix)
            for suffix in ("App.swift", "AppDelegate.swift", "SceneDelegate.swift", "main.swift")
        ):
            return True
    ...
    def _identify_public_apis(self) -> None:
        """Phase 9: Identify the declared public API surface.

        The sibling modules grep the source for an ``export`` keyword, because
        neither Python nor TypeScript records visibility anywhere the graph can
        see it. Swift states access level with a keyword on the declaration,
        and the extractor stores it, so this phase is a SQL query over data
        already in the graph rather than a second, weaker parse of the source.

        That is not just tidier — it is more accurate. A grep cannot tell a
        ``public`` declaration from the word "public" in a comment, cannot see
        an access level written on the enclosing extension, and cannot
        distinguish ``public`` from ``open``.
        """
        try:
            ...
            rows = con.execute(
                ...
                SELECT id, name, kind, module_path, docstring,
                       json_extract(metadata, '$.visibility') AS visibility
                FROM nodes
                WHERE json_extract(metadata, '$.visibility') IN ('public', 'open')
                  AND id NOT LIKE 'sym:%'
                """
            ).fetchall()

            for node_id, nm, kind, module_path, docstring, _visibility in rows:
            ...
            # Supplement from high fan-in metrics: a declaration reached from
            # several modules is part of the effective API whatever it says.
            for func in sorted(
                self.function_metrics.values(), key=lambda m: m.fan_in, reverse=True
            ):
                if func.fan_in >= 1 and func.node_id not in already_ids:
                    self.public_apis.append(func)
                    already_ids.add(func.node_id)

            self.public_apis.sort(key=lambda m: m.fan_in, reverse=True)
            self._phase_result = f"{len(self.public_apis)} public declarations"

        except (AttributeError, ValueError, RuntimeError) as exc:
            logger.warning("Public API identification incomplete: %s", exc)

    def _analyze_doc_comment_coverage(self) -> None:
        """Phase 10: Measure doc-comment coverage across all node kinds."""
        try:
            con = self.kg.store.con
            rows = con.execute(
                """
                ...
                FROM nodes
                WHERE kind IN ('function', 'method', 'class', 'struct', 'enum',
                               'protocol', 'actor', 'extension', 'property',
                               'typealias', 'module')
                GROUP BY kind
                ORDER BY kind
                """
            ...
            logger.warning("doc-comment coverage analysis incomplete: %s", exc)

    def _analyze_inheritance(self) -> None:
        """Phase 11: Analyze the type hierarchy.

        Processes three edge types:
        - INHERITS: class or actor derives from a superclass
        - CONFORMS: any type or extension conforms to a protocol
        - EXTENDS: an extension augments a type

        Depth is computed over INHERITS alone. Swift conformance is not a
        depth relation — a type may conform to any number of protocols without
        that saying anything about how deep its class hierarchy runs — and
        folding conformances into the depth calculation would report a flat
        struct-and-protocol codebase as deeply nested.
        """
        try:
            con = self.kg.store.con
            ...
            conforms_rows = con.execute(
                "SELECT src, dst FROM edges WHERE rel = 'CONFORMS'"
            ).fetchall()
            extends_rows = con.execute(
                "SELECT src, dst FROM edges WHERE rel = 'EXTENDS'"
            ).fetchall()

            total_edges = len(inherits_rows) + len(conforms_rows) + len(extends_rows)

            if total_edges == 0:
                self.inheritance_analysis = {
                    "total_inherits_edges": 0,
                    "total_conforms_edges": 0,
            ...
            # Implements table
            conforms_list: list[dict] = []
            for src, dst in conforms_rows:
                ...
                proto_name = dst_node.get("name", dst) if dst_node else dst.split(":")[-1]
                cls_mod = src_node.get("module_path", "") if src_node else ""
                conforms_list.append({"type": cls_name, "protocol": proto_name, "module": cls_mod})

            self.inheritance_analysis = {
                "total_inherits_edges": len(inherits_rows),
                "total_conforms_edges": len(conforms_rows),
                ...
                "conforms": conforms_list,
            }

            self._phase_result = (
                f"{len(all_classes)} types  max-depth={max_depth}  "
                f"{len(conforms_rows)} conformances  {len(extends_rows)} extensions"
            )
    ...
    def _generate_insights(self) -> None:
        ...
        # doc-comment coverage signals
        cov = self.doc_comment_coverage
        if cov:
            pct = cov["coverage_pct"]
            if pct >= 80:
                self.strengths.append(
                    f"Good doc-comment coverage: {pct}% of declarations documented — "
                    "semantic retrieval will be effective"
                )
            elif pct >= 50:
                self.issues.append(
                    f"[WARN] Moderate doc-comment coverage ({pct}%) — semantic retrieval is degraded "
                    "for undocumented nodes; prioritize high-fan-in functions first"
                )
            else:
                self.issues.append(
                    f"[LOW] Low doc-comment coverage ({pct}%) — semantic query quality will be poor; "
        ...
        if inh:
            if inh.get("max_depth", 0) > 4:
                self.issues.append(
                    ...
                    "prefer composition over deep inheritance in Swift"
                )
            ...
            if inh.get("conforms"):
                self.strengths.append(
                    f"{len(inh['conforms'])} protocol conformances — "
                    "protocol-oriented design in use"
                )
    ...
    def _compute_quality_grade(self) -> tuple[float, str, str]:
        ...
        Scoring (100 points):
        - doc-comment coverage (0–40 pts): ≥80% → 40, ≥50% → 20, else 0
        - Orphaned declarations (0–25 pts): 0 → 25, 1–2 → 15, 3–5 → 5, else 0
        - High fan-out functions (0–20 pts): 0 → 20, 1–2 → 12, else 4
        - Type safety signals (0–15 pts): conformance edges present → 15, else 0
        """
        score = 0.0
        ...
        inh = self.inheritance_analysis
        if inh and inh.get("total_conforms_edges", 0) > 0:
            score += 15
    ...
    def _build_recommendations(self) -> str:
        ...
        if inh and inh.get("max_depth", 0) > 3:
            long_term.append(
                ...
                "over deep class hierarchies in Swift"
            )
        ...
        if not immediate and not medium:
            immediate.append(
                "**Maintain current quality** — no critical issues detected; keep doc-comment "
    ...
    def _get_report_metadata(self, elapsed_seconds: float = 0.0) -> str:
        ...
        try:
            ...
            version = f"swift-kg {_pkg_version('swift-kg')}"
        except Exception:  # noqa: BLE001
            version = "swift-kg (dev)"
...
Comprehensive architectural analysis of **{repo_name}** using SwiftKG's knowledge graph.
Covers complexity hotspots, module coupling, call chains, doc-comment coverage, and type hierarchy.
...
| **Structs** | {stats.get("node_counts", {}).get("struct", "N/A")} |
| **Protocols** | {stats.get("node_counts", {}).get("protocol", "N/A")} |
| **Actors** | {stats.get("node_counts", {}).get("actor", "N/A")} |
| **Extensions** | {stats.get("node_counts", {}).get("extension", "N/A")} |
| **Enums** | {stats.get("node_counts", {}).get("enum", "N/A")} |
| **Properties** | {stats.get("node_counts", {}).get("property", "N/A")} |
| **Type Aliases** | {stats.get("node_counts", {}).get("typealias", "N/A")} |
...
| CONFORMS | {stats.get("edge_counts", {}).get("CONFORMS", 0)} |
...
clear doc-comment contracts, and stable protocol boundaries.
...
"""
        ...
        if cov:
            ...
            report += "---\n\n## doc-comment Coverage\n\n"
            report += (
                "doc-comment coverage determines semantic retrieval quality. Nodes without doc-comment "
                "embed only structured identifiers — keyword search is as effective as vector "
                "embeddings. The semantic model earns its value only when doc-comment is present.\n\n"
            )
...
## Type Hierarchy and Conformance

"""
        inh = self.inheritance_analysis
        if inh and (inh.get("total_inherits_edges", 0) + inh.get("total_conforms_edges", 0)) > 0:
            report += (
                f"**{inh.get('total_inherits_edges', 0)}** INHERITS · "
                f"**{inh.get('total_conforms_edges', 0)}** CONFORMS · "
                f"**{inh.get('total_extends_edges', 0)}** EXTENDS\n\n"
            )
            if inh.get("classes"):
                report += "### Class and Actor Hierarchy (INHERITS)\n\n"
                report += "| Type | Module | Depth | Parents | Children |\n|---|---|---|---|---|\n"
            ...
            if inh.get("conforms"):
                ...
                report += "| Type | Protocol | Module |\n|---|---|---|\n"
                for impl in inh["conforms"][:15]:
                    report += f"| `{impl['type']}` | `{impl['protocol']}` | {impl['module']} |\n"
                report += "\n"
            if inh.get("multiple_inheritance"):
                report += f"### Multiple Base Types ({len(inh['multiple_inheritance'])} types)\n\n"
        ...
        if self.snapshot_history:
            ...
            report += "No snapshots. Run `swiftkg snapshot save <version>` to capture one.\n"
        ...
        report += (
            f"\n\n---\n\n*Report generated by SwiftKG analysis — completed in {elapsed_str}*\n"
        )
    ...
    def _render_phase_failures(self) -> str:
        ...
        if any(f.get("missing_index") for f in self.phase_failures):
            lines.append(
                ...
                "rest of this report is complete. Run `swiftkg build --repo <path>` "
                "(or `swiftkg build-index`) to build it, then re-run the analysis."
            )
        lines.append("")
        return "\n".join(lines)
    ...
    def to_markdown(self) -> str:
        ...
        out.append("# SwiftKG Repository Analysis\n")
        ...
        out.append(f"| Structs | {stats.get('node_counts', {}).get('struct', 'N/A')} |")
        out.append(f"| Protocols | {stats.get('node_counts', {}).get('protocol', 'N/A')} |")
        out.append(f"| Actors | {stats.get('node_counts', {}).get('actor', 'N/A')} |")
        out.append(f"| Extensions | {stats.get('node_counts', {}).get('extension', 'N/A')} |")
        out.append("")

        out.append("### Edge Distribution")
        out.append("| Relationship | Count |")
        out.append("|---|---|")
        for rel in ("CALLS", "CONTAINS", "IMPORTS", "INHERITS", "CONFORMS", "EXTENDS"):
        ...
        out.append("## Type Hierarchy and Conformance\n")
        inh = self.inheritance_analysis
        if inh and inh.get("total_inherits_edges", 0) > 0:
            ...
            out.append("| Type | Module | Depth | Parents | Children |")
            out.append("|---|---|---|---|---|")
