"""
cli/cmd_hooks.py — swiftkg install-hooks command.
...
# installed as a package in any repo, not just swift_kg itself)
# ---------------------------------------------------------------------------

_PRE_COMMIT_HOOK = """\
#!/usr/bin/env bash
# SwiftKG pre-commit hook — runs quality checks. The index rebuild and
# metrics snapshot are opt-in and OFF by default; see "Why snapshots are off".
# Installed by: swiftkg install-hooks
#
#   SWIFTKG_SNAPSHOT=1 git commit ...        opt in to a per-commit snapshot
#   SWIFTKG_SKIP_SNAPSHOT=1 git commit ...   force snapshots off (wins)
#
# Note that SWIFTKG_SKIP_SNAPSHOT no longer skips the quality checks. It used
...
# SWIFTKG_SNAPSHOT=1 is set, and is skipped regardless if
# SWIFTKG_SKIP_SNAPSHOT=1.
# ---------------------------------------------------------------------------
[ "${SWIFTKG_SNAPSHOT:-0}" = "1" ] || exit 0
[ "${SWIFTKG_SKIP_SNAPSHOT:-0}" = "1" ] && exit 0

# Resolve the swiftkg binary: prefer the repo's .venv, fall back to PATH.
if [ -x "$REPO_ROOT/.venv/bin/swiftkg" ]; then
    SWIFTKG="$REPO_ROOT/.venv/bin/swiftkg"
elif command -v swiftkg &>/dev/null; then
    SWIFTKG="swiftkg"
else
    echo "[swiftkg] binary not found — skipping snapshot" >&2
    exit 0
fi
...
"$SWIFTKG" build --repo "$REPO_ROOT" || exit 1

# Snapshot SwiftKG (version auto-detected from installed package).
"$SWIFTKG" snapshot save \\
    --repo . \\
    --tree-hash "$TREE_HASH" \\
    --branch "$BRANCH" \\
  || { echo "[swiftkg] snapshot skipped (run 'swiftkg build' to initialize)" >&2; }
...
git add .swiftkg/snapshots/ 2>/dev/null || true
...
def install_hooks(repo: str, force: bool) -> None:
    """Install the SwiftKG pre-commit git hook.

    After installation, before each commit:

    \b
      1. Rebuilds the local SwiftKG index (full wipe)
    ...
    click.echo("  Opt in with:  SWIFTKG_SNAPSHOT=1 git commit ...")
    click.echo("  Force off:    SWIFTKG_SKIP_SNAPSHOT=1 git commit ...")
    click.echo("  Run 'swiftkg build' first if you haven't built the graph yet.")
