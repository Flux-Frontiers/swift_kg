"""
cli/cmd_snapshot.py — swiftkg snapshot subcommands.

Manage temporal snapshots of SwiftKG metrics:
...
def _default_snapshots_dir(snapshots_dir: str | None, repo_root: Path | None = None) -> Path:
    ...
    return Path(snapshots_dir).resolve() if snapshots_dir else base / ".swiftkg" / "snapshots"
...
@click.option(
    ...
    default=None,
    type=click.Path(),
    help="Snapshots directory (default: <repo>/.swiftkg/snapshots).",
)
@click.option(
    "--branch",
    default=None,
...
@click.option(
    ...
    default="",
    type=str,
    help="What was measured, e.g. 'repo:swift-kg'.",
)
def save_snapshot(
    version: str | None,
    repo: str,
    ...
    Snapshots are stored in .swiftkg/snapshots/{key}.json, with a
    manifest.json tracking all snapshots and their metrics.

    Example:
        swiftkg snapshot save 0.1.0 --repo . --subject repo:swift-kg
    """
    capture_snapshot(
        version=version,
        repo=repo,
...
def capture_snapshot(
    *,
    ...
    :param db: SQLite graph path; defaults to ``<repo>/.swiftkg/graph.sqlite``.
    :param snapshots_dir: Snapshots directory; defaults to ``<repo>/.swiftkg/snapshots``.
    ...
    :param subject: What was measured, e.g. ``repo:swift-kg``. Recorded
    ...
    from swift_kg.snapshots import SnapshotManager  # pylint: disable=import-outside-toplevel

    repo_root = Path(repo).resolve()
    db_path = Path(db) if db else repo_root / ".swiftkg" / "graph.sqlite"
    snapshots_path = _default_snapshots_dir(snapshots_dir, repo_root)

    kg = SwiftKG(repo_root=repo_root, db_path=db_path)
    ...
    try:
        ...
        try:
            ...
            analyzer = SwiftKGAnalyzer(kg, snapshot_mgr=snap_mgr)
...
@click.option(
    ...
    default=None,
    type=click.Path(exists=True),
    help="Snapshots directory (default: .swiftkg/snapshots).",
)
@click.option(
    "--limit",
    type=int,
...
def diff_snapshots(key_a: str, key_b: str, snapshots_dir: str | None, output_json: bool) -> None:
    ...
    Example:
        swiftkg snapshot diff 660e4f0a 3487ed5b
    """
    from swift_kg.snapshots import SnapshotManager  # pylint: disable=import-outside-toplevel
...
def prune_snapshots(snapshots_dir: str | None, dry_run: bool) -> None:
    ...
    Example:
        swiftkg snapshot prune --dry-run
        swiftkg snapshot prune
    """
    from swift_kg.snapshots import SnapshotManager  # pylint: disable=import-outside-toplevel
