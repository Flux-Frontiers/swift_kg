"""
cli/cmd_init.py — swiftkg init command.
...
from swift_kg.cli.cmd_hooks import _PRE_COMMIT_HOOK


def _has_swiftkg_config(repo_root: Path) -> bool:
    """Check whether pyproject.toml already has a [tool.swiftkg] section.
    ...
    try:
        with open(pyproject, "rb") as f:
            data = tomllib.load(f)
        return "swiftkg" in data.get("tool", {})
    except (OSError, ValueError):
        return False


def _scaffold_swiftkg_config(repo_root: Path) -> bool:
    """Append a minimal [tool.swiftkg] section to pyproject.toml if missing.
    ...
    # SwiftPM lays a package out as Sources/ and Tests/; Xcode projects vary
    # more, so App/ and Source/ cover the common hand-rolled layouts.
    candidates = ["Sources", "Source", "App", "src"]
...
def init(
    ...
    """Initialize SwiftKG in a repository.
    ...
    click.echo()
    click.echo("  SwiftKG  Init")
    click.echo(f"  repo  {repo_root}")
    click.echo()

    # ------------------------------------------------------------------
    # Step 0: Scaffold [tool.swiftkg] in pyproject.toml if missing
    # ------------------------------------------------------------------
    if not _has_swiftkg_config(repo_root):
        if _scaffold_swiftkg_config(repo_root):
            click.echo("  [0/4]  Added [tool.swiftkg] section to pyproject.toml")
        else:
            click.echo("  [0/4]  No pyproject.toml found — skipping config scaffold")
    else:
        click.echo("  [0/4]  [tool.swiftkg] config already present")
    ...
    kg = SwiftKG(repo_root=repo_root, model=model)
    try:
    ...
    if skip_snapshot:
        ...
        from swift_kg.cli.cmd_snapshot import (  # pylint: disable=import-outside-toplevel
            capture_snapshot,
        )
    ...
    click.echo()
    click.echo(f"  Done in {elapsed:.1f}s — SwiftKG is ready.")
    click.echo('  Try:  swiftkg query "networking layer"')
