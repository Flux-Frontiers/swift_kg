"""
Framework Detector for SwiftKG.
...
exclusion list adapted to Swift lifecycle and protocol members.
...
# Member names that are structurally high fan-in but architecturally trivial.
# Excluded from framework-node ranking so hubs surface over boilerplate. Swift
# skews this harder than most languages: protocol witnesses (`description`,
# `hashValue`, `encode`) and SwiftUI's `body` appear on nearly every type, and
# an unfiltered ranking returns those instead of the repo's actual hubs.
...
def detect_framework_nodes(
    limit: int = 25, db_path: str = "swiftkg.sqlite"
    ...
    Swift lifecycle and protocol-witness boilerplate (``init``, ``deinit``,
    ``description``, ``body``, ``viewDidLoad``, …) is excluded so the ranking
    surfaces meaningful orchestrators rather than the most-conformed-to
    protocol witnesses.

    :param limit: Number of top framework nodes to return (default 25).
    :param db_path: Path to SQLite database (default "swiftkg.sqlite").
    ...
    # bare module path (e.g. "Sources/Networking/Client.swift"), which we map to "mod:..."
    # IDs for output.
