#!/usr/bin/env python3
"""
mcp_server.py — SwiftKG MCP Server

Exposes the SwiftKG hybrid query and snippet-pack pipeline as
Model Context Protocol (MCP) tools, allowing any MCP-compatible agent
(Claude Desktop, Cursor, Continue, etc.) to query a Swift
...
type_hierarchy(node_id)
    Conformers, subclasses, extensions and declared supertypes of one type.

public_api(module_path, limit)
    The declared `public` / `open` API surface, read from access levels.
...
def _get_kg() -> SwiftKG:
    if _kg is None:
        raise RuntimeError(
            "SwiftKG not initialised. Run the server via 'swiftkg-mcp --repo /path/to/repo'"
        )
    return _kg
...
def _get_snapshot_mgr() -> SnapshotManager:
    if _snapshot_mgr is None:
        raise RuntimeError(
            "SnapshotManager not initialised. Run the server via 'swiftkg-mcp --repo /path/to/repo'"
        )
...
async def _lifespan(_server: FastMCP) -> AsyncIterator[None]:
    """Close the long-lived SwiftKG instance when the server shuts down.

    Per FLEET_STANDARDS (settled 2026-08-24): ``KGModule`` owns a SQLite
    handle, and leaking it to process exit is merely untidy for a one-shot
    command but a real leak for a long-lived server. The stdio and SSE
    transports both route through the same ``Server.run()``, so this one hook
    covers both.
    """
    try:
        yield
...
mcp = FastMCP(
    ...
    lifespan=_lifespan,
    instructions=(
        "SwiftKG is a hybrid semantic + structural knowledge graph for Swift and "
        "Swift codebases. It indexes every file, class, struct, enum, protocol, actor, "
        "extension, function, method, property and type alias as a node, with typed edges "
        "(CALLS, IMPORTS, CONTAINS, INHERITS, CONFORMS, EXTENDS) connecting them.\n\n"
        "## Tools\n\n"
        "**graph_stats()** — Start here. Returns node/edge counts by kind and relation "
        "plus doc-comment coverage. Use before issuing query_codebase() or pack_snippets().\n\n"
        "**query_codebase(q, k, hop, rels, max_nodes, min_score, max_per_module, rerank_mode)** — "
        ...
        "Filter with paths='Sources/' to exclude test callers.\n\n"
        "**type_hierarchy(node_id)** — Everything about one Swift type at once: its "
        "conformers (if a protocol), subclasses, the extensions declared on it — which in "
        "Swift routinely live in other files — and what it conforms to and inherits from. "
        "Start here for any question about a type or protocol.\n\n"
        "**public_api(module_path, limit)** — The declared `public` / `open` surface, read "
        ...
        "**analyze_repo()** — Structural analysis: node counts, edge counts, doc-comment coverage, "
        "node distribution by kind.\n\n"
        "**explain(node_id, limit)** — Natural-language Markdown explanation of a node: "
        "metadata, doc-comment, callers, callees, and its role in the codebase.\n\n"
        ...
        "- **Explore unfamiliar Swift repo**: graph_stats → query_codebase → pack_snippets\n"
        "- **Find a specific class/function**: find_node(name) → get_node(include_edges=True)\n"
        "- **Understand a protocol**: get_node → callers(rel='CONFORMS')\n"
        "- **Trace usage of a symbol**: find_node(name) → callers(node_id)\n"
        "- **Understand a Swift type**: find_node(name) → type_hierarchy(node_id)\n"
...
def query_codebase(
    ...
    k: int = 8,
    hop: int = 1,
    rels: str = "CONTAINS,CALLS,IMPORTS,INHERITS,CONFORMS,EXTENDS",
    max_nodes: int = 25,
    min_score: float = 0.0,
    ...
    :param q: Natural-language query, e.g. "request retry policy".
...
def pack_snippets(
    ...
    :param q: Natural-language query, e.g. "request error handling".
    :param k: Number of semantic seed nodes (default 8).
    :param hop: Graph expansion hops (default 1).
    :param rels: Comma-separated edge types to follow.
...
@mcp.tool()
def type_hierarchy(node_id: str) -> str:
    """
    Return everything the graph knows about one Swift type's relationships.

    Swift spreads a type across three relations that the sibling language
    modules do not have to separate, and answering "what is this type" means
    reading all three at once:

    - **conformers** — every type and extension conforming to it, if it is a
      protocol. This is the question about a protocol.
    - **subclasses** — every direct subclass, if it is a class or actor.
    - **extensions** — every ``extension`` declared on it, which in Swift
      routinely live in other files. "Where is the rest of this type" is a real
      question with a graph answer.
    ...
    def _outgoing(rel: str) -> list:
        try:
            edges = kg.store.edges_from(node_id, rel=rel, limit=200)
        except (AttributeError, ValueError, RuntimeError):
            return []
        out = []
        for edge in edges:
            raw = edge.get("dst") if isinstance(edge, dict) else edge[1]
            if not raw:
                continue
            dst = str(raw)
            target = None if dst.startswith("sym:") else kg.node(dst)
            out.append(target or {"id": dst, "name": dst.split(":")[-1], "external": True})
        return out

    return json.dumps(
        {
            "node": node,
            "conformers": kg.conformers(node_id),
            "subclasses": kg.subclasses(node_id),
            "extensions": kg.extensions_of(node_id),
            "conforms_to": _outgoing("CONFORMS"),
            "inherits_from": _outgoing("INHERITS"),
        },
        indent=2,
        ensure_ascii=False,
    )
...
@mcp.tool()
def public_api(module_path: str = "", limit: int = 100) -> str:
    """
    ...
    params: list = []
    if module_path:
        sql += " AND module_path LIKE ?"
        params.append(f"{module_path}%")
    sql += " ORDER BY module_path, qualname LIMIT ?"
    params.append(limit)
    ...
    return json.dumps(
        {"count": len(declarations), "declarations": declarations}, indent=2, ensure_ascii=False
    )
...
def graph_stats() -> str:
    ...
    Call this first when engaging with a new Swift repo.
    Reports doc-comment coverage (fraction of functions/methods with doc-comment comments).

    :return: Markdown summary with total counts, nodes-by-kind, and edges-by-relation tables.
    """
    stats = _get_kg().stats()
    out: list[str] = ["## SwiftKG Graph Statistics\n"]
    ...
    if cov is not None:
        out.append(f"- **doc-comment coverage:** {cov:.1%} *(functions + methods)*")
    out.append("")
...
def list_nodes(
    ...
    :param module_path: Module path prefix filter (e.g. "Sources/Networking/Client.swift").
    :param kind: Node kind filter: module | class | struct | enum | protocol |
...
def find_node(name: str, kind: str = "", limit: int = 100) -> str:
    ...
    :param name: Function, type, or protocol name to search for.
...
def centrality(
    ...
    (CALLS > INHERITS/CONFORMS/EXTENDS > IMPORTS > CONTAINS) and amplified for
...
def find_definition_at(file: str, line: int) -> str:
    ...
    rows = store.con.execute(
        ...
        WHERE (module_path = :f OR module_path LIKE :like)
          AND kind IN ('function', 'method', 'class', 'struct', 'enum', 'protocol',
                       'actor', 'extension', 'property', 'typealias')
    ...
    if not rows:
        ...
        if not mod_rows:
            return (
                ...
                f"No function, method, type, extension or property spans "
...
def analyze_repo() -> str:
    ...
    Executes the 14-phase SwiftKG analysis pipeline — baseline metrics,
    CodeRank, fan-in/fan-out, module coupling, critical call chains, public API
    surface, doc-comment coverage, type hierarchy and conformance, insights, snapshot
    ...
    kg = _get_kg()
    try:
        analyzer = SwiftKGAnalyzer(kg, console=silent, snapshot_mgr=_snapshot_mgr)
...
def rank_nodes(
    top: int = 25,
    rels: str = "CALLS,IMPORTS,INHERITS,CONFORMS,EXTENDS",
    persist_metric: str = "",
    ...
    INHERITS/CONFORMS/EXTENDS=0.75.  Test paths are excluded by default.
...
def explain_rank(node_id: str, q: str = "") -> str:
    ...
    if node_id in graph:
        ...
        inheritors_count = sum(
            ...
            if d.get("relations", set()) & {"INHERITS", "CONFORMS", "EXTENDS"}
        )
        if callers_count:
...
def snapshot_list(limit: int = 10, branch: str = "") -> str:
    ...
    "when did doc-comment coverage improve?" or "show me only main-branch snapshots".
...
def _parse_args(argv: list | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        prog="swiftkg-mcp",
        description="SwiftKG MCP server — exposes Swift codebase query tools to AI agents.",
    )
    p.add_argument("--repo", default=".", help="Repository root directory (default: .)")
    p.add_argument(
        "--db",
        default=".swiftkg/graph.sqlite",
        help="Path to the SQLite knowledge graph",
    )
    p.add_argument(
        "--vectors",
        default=".swiftkg/vectors.sqlite",
...
def main(argv: list | None = None) -> None:
    """CLI entry point for the SwiftKG MCP server."""
    ...
    if not db.exists():
        print(
            f"WARNING: SQLite database not found at '{db}'.\nRun 'swiftkg build --repo .' first.",
            file=sys.stderr,
        )

    print(
        f"SwiftKG MCP server starting\n"
        f"  repo     : {repo}\n"
        f"  db       : {db}\n"
        f"  vectors  : {vectors}\n"
        f"  model    : {args.model}\n"
    ...
    _kg = SwiftKG(repo_root=repo, db_path=db, vectors_path=vectors, model=args.model)
    _snapshot_mgr = SnapshotManager(repo / ".swiftkg" / "snapshots", db_path=db)
