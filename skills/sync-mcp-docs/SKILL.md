---
name: sync-mcp-docs
description: Synchronize all SwiftKG MCP documentation with the runtime tool API in src/swift_kg/mcp_server.py — the module docstring "Tools" list, the FastMCP instructions block, README.md, and the swiftkg skill files. This is a required project rule; any change to MCP tool signatures, parameters, defaults, or behavior must ship with matching doc updates in the same commit. Use this skill when the user says: "sync the MCP docs", "update the tool descriptions", "the mcp_server docstring is stale", "keep the FastMCP instructions in sync", "I added/renamed/removed an MCP tool", or after any edit to @mcp.tool() functions in mcp_server.py.
---

# Sync MCP Documentation

You are updating all SwiftKG MCP documentation and provider instructions to reflect the current state of `src/swift_kg/mcp_server.py`. Execute the following steps in order.

> **Project rule (from CLAUDE.md):** Any change to MCP tool signatures, parameters, defaults, or behavior in `src/swift_kg/mcp_server.py` must include a matching update to the `mcp = FastMCP(..., instructions=(...))` tool descriptions **in the same commit**. Keep the module docstring "Tools" list and the `FastMCP` instructions block aligned with the runtime tool API.

---

## Step 0: Establish the Source of Truth

Read `src/swift_kg/mcp_server.py` and extract the **authoritative tool list**:
...
The docstring at the top of `src/swift_kg/mcp_server.py` has a `Tools` section listing every tool as:
...
### 2c. `skills/swiftkg/references/CHEATSHEET.md`
...
1. Stage all modified files:
   ```bash
   git add src/swift_kg/mcp_server.py README.md \
           skills/swiftkg/SKILL.md \
           skills/swiftkg/references/CHEATSHEET.md
   ```
...
2. Write `commit.txt` with a conventional commit message:
   ...
   Files updated:
   - src/swift_kg/mcp_server.py: docstring Tools list + FastMCP instructions
   - README.md: ...
   - skills/swiftkg/SKILL.md: ...
   - skills/swiftkg/references/CHEATSHEET.md: ...
   ```
...
✓ skills/swiftkg/SKILL.md  — updated (frontmatter + tool tables)
